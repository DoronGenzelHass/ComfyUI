...
def get_valid_token_sync() -> Optional[Token]:
    """Synchronous helper: returns cached token without refreshing.

    Used in code paths that aren't already in an event loop. The async
    ``HF_AUTH_STORE.get_valid_token()`` is preferred because it can
    refresh, but this exists for places where we can't await.
    """
    return HF_AUTH_STORE.get_token_sync()


def get_valid_token_async_in_loop() -> asyncio.Task:
    """Schedule a token-fetch task on the running loop. Internal helper."""
    return asyncio.create_task(HF_AUTH_STORE.get_valid_token())
